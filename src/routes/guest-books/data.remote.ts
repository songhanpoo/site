...
export const insertGuestBook = command('unchecked', async ({ content }: { content: string }) => {
  const { locals } = getRequestEvent();
  if (!locals.user) error(401, 'Unauthorized');
  if (!content || content.trim().length < 3 || content.length > 140) error(400, 'Invalid content length');
